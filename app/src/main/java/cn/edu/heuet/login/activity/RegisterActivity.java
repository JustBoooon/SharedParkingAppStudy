...
//                asyncRegister(telephone, otpCode, username, gender, age, password1, password2);
    ...
    /**
     * XHttp2异步校验，有学习成本，但是简洁
     * 目前用的是这个，后面毕设搞完了可以回去看OkHttp
     * 在这里发起了post请求
     *
     * @param telephone 手机号
     */
    private void asyncGetOtpCodeWithXHttp2(String telephone) {
        //获取验证码的URL，和后端controller对应
        XHttp.post(NetConstant.getGetOtpCodeURL())
                .params("telephone", telephone)
                //异步声明，默认就是false，不声明也可以，这里只是显示出来了
                .syncRequest(false)
                //接收的和返回的回调要匹配
        ...
        XHttp.post(NetConstant.getRegisterURL())
                ...
                .execute(new SimpleCallBack<Object>() {
                    ...
                    public void onSuccess(Object data) throws Throwable {
                        ...
                        editor = sp.edit();
                        //把telephone存入SharedPreferences
                        editor.putString("telephone", telephone);
                        //通过MD5加密
                        String encryptedPassword = ValidUtils.encodeByMd5(password1);
                        //把加密过的encryptedPassword存入SharedPreferences
                        editor.putString("encryptedPassword", encryptedPassword);

                        //提交后跳转至主页
                        if (editor.commit()) {
                            Intent intentToMain = new Intent(RegisterActivity.this, MainActivityPast.class);
    ...
    /**
     * OkHttp异步请求验证码，较为复杂，但是学习成本较低，死记硬背四步走
     * @param telephone
     */
                        ...
                        public void onResponse(Call call, Response response) throws IOException {
                            ...
                            if (responseStr.contains("200")) {
                                ...
                                if (getResponseStatus(responseBodyJSONObject).equals("success")) {
                                    ...
                                    if (editor.commit()) {
                                        Intent it_register_to_main = new Intent(RegisterActivity.this, MainActivityPast.class);
